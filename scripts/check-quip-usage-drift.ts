...
function withinGracePeriod(): boolean {
  if (!CUSTOM_DIMENSIONS_REGISTERED_AT) return true; // 아직 등록 안 함 — 계속 유예
  const registeredAt = Date.parse(`${CUSTOM_DIMENSIONS_REGISTERED_AT}T00:00:00Z`);
  const elapsedDays = (Date.now() - registeredAt) / 86_400_000;
...
async function main() {
  if (withinGracePeriod()) {
    console.log(
      `유예 기간(맞춤 측정기준 등록 후 ${GRACE_PERIOD_DAYS}일, 또는 아직 미등록) — 건너뜀`,
    );
    return;
  }
