  ...
  const labels = scores
    .map((s, i) => {
      ...
      return (
        ...
        `<tspan fill="${s.pct === null ? '#6b7280' : color}" font-size="10"> ${val}</tspan></text>`
      );
    })
    .join('');
...
async function main() {
  ...
  for (const [i, id] of ids.entries()) {
    ...
    const scores = hexScores(records, HEX_BASELINE);
    cards.push(
    ...
    notes.push(
      `${(myName || id).padEnd(14)} ` +
        scores
          .map((s) => `${s.label} ${s.pct === null ? '—' : Math.round(s.pct)}`)
          .join('  '),
    );
  }
...
${cards.join('\n')}
<text x="12" y="280" font-size="10" fill="#6b7280">숫자 = 백분위 (전체 플레이어 ${HEX_BASELINE_N}명 기준). 100에 가까울수록 그 항목이 남들보다 높다는 뜻.</text>
</svg>`;
