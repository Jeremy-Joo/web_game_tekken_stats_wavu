...
function radar(p: P, hue: string): string {
  ...
  const shape = p.scores
    .map((s, i) => pol(C, C, (R * (s.value ?? 50)) / 100, i, n).map(f1).join(','))
    .join(' ');
  const grid =
    [0.25, 0.5, 0.75, 1]
      .map(
  ...
  const labels = p.scores
    .map((s, i) => {
      ...
      return `<text x="${f1(x)}" y="${f1(y + 4)}" text-anchor="${anchor}" class="ax">${s.label} <tspan class="axv">${s.value === null ? '—' : Math.round(s.value)}</tspan></text>`;
    })
    .join('');
...
function glyph(p: P, hue: string): string {
  ...
  const bars = p.scores
    .map((s, i) => {
      const r = Math.max((R * (s.value ?? 0)) / 100, 6);
      const [x, y] = pol(C, C, r, i, n);
      return `<line x1="${C}" y1="${C}" x2="${f1(x)}" y2="${f1(y)}" stroke="${s.value === null ? 'var(--muted)' : hue}" stroke-width="13" stroke-linecap="round"><title>${s.label} ${s.value === null ? '표본 부족' : `${Math.round(s.value)}백분위`}</title></line>`;
    })
    .join('');
  ...
  return HEX_AXES.map((ax, ai) => {
    ...
    const marks = players
      .map((p, i) => {
        ...
        return `<g><line x1="${X(s.raw).toFixed(2)}%" x2="${X(s.raw).toFixed(2)}%" y1="8" y2="36" stroke="${HUES[i]}" stroke-width="3"><title>${p.name} · ${s.value === null ? '' : Math.round(s.value) + '백분위'}</title></line></g>`;
      })
      .join('');
...
async function main() {
  ...
  for (const id of ids) {
    ...
    players.push({ name: myName || id, records, scores: hexScores(records) });
  }
