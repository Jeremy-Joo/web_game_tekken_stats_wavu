...
/**
 * export 하는 이유: player-index 의 '장기전 상승' 판정(build-player-index.ts)이
 * 이 값을 그대로 대칭으로 재사용한다(하락 기준을 복사해두면 나중에 여기만 바꾸고
 * 거긴 안 바뀌는 사고가 난다). **계산식 자체는 안 바뀐다** — 값을 꺼내 쓸 수만
 * 있게 한 것.
 */
export const DROP_PP = 3;
