 ...
 * 경기당 변동이 ±8~13(2026-08-05 실측, docs/rating-threshold-research.md)이라
 * 150이면 두 자릿수 순연패 급 — 작은 출렁임까지 회복 서사로 부풀리지 않기 위한 하한이다.
 */
...
/** 순변화가 이 안이면 '제자리'다. 경기당 변동이 ±8~13(위 RECOVERY_DIP 주석 참조)이라 한 판 값보다 크게 잡았다. */
...
export interface QuipFacts {
   ...
   * "이기고 있다"/"5할 이하"의 기준은 고정값이 아니라 레이팅 구간별 손익분기다
   * (breakevenWinRate 참조, docs/rating-threshold-research.md).
   *   winNoGain   그 구간 손익분기보다 뚜렷이 이기고 있는데 레이팅은 제자리거나 후퇴
   *   loseButGain 손익분기보다 뚜렷이 밑도는데 레이팅은 올랐다 — 위를 잡고 아래에 진 것
   *   flatEven    손익분기보다 뚜렷이 밑돌고 + 레이팅도 제자리 — 시스템이 판단을 보류 중
