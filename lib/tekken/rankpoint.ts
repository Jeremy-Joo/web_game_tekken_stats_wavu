...
export function estimateRankPoints(matches: RankMatch[]): RankPointEstimate {
  ...
  if (!model) {
    return { ...none('이 단의 점수 모델이 아직 없음'), sinceAnchor };
  }
  ...
  if (confidence === 'none') {
    return {
      ...none(
        clampedBy > errorMargin
          ? '추정값이 구간을 벗어남 — 경기 기록이 빠졌을 수 있음'
          : '기준점에서 너무 멀어 오차가 큼',
      ),
      sinceAnchor,
    };
 ...
 *   그 이상       → 약 41만
 */
export function formatScore(score: number, errorMargin: number): string {
  if (errorMargin < 1000) return score.toLocaleString('ko-KR');
  if (errorMargin < 5000) return `약 ${(Math.round(score / 1000) * 1000).toLocaleString('ko-KR')}`;
  return `약 ${(Math.round(score / 10000) / 10).toFixed(1).replace(/\.0$/, '')}만`;
}
