...
export const HEX_AXES: HexAxis[] = [
  ...
  },
  {
    key: 'variety',
    label: '다양성',
    desc: '고르게 썼다면 몇 캐릭에 해당하는가. 주력 비중이 높을수록 작아진다.',
    calc: (rs) => {
      const count = new Map<string, number>();
      for (const r of rs) count.set(r.myChar, (count.get(r.myChar) ?? 0) + 1);
      if (count.size === 0) return null;
      if (count.size === 1) return 1; // 한 캐릭만 → 실질 1캐릭 (정의상 정확히 1)
      let h = 0;
      for (const v of count.values()) {
        const p = v / rs.length;
        h -= p * Math.log(p);
      }
      // exp(엔트로피) = **실질 캐릭터 수**. 엔트로피를 사람이 읽을 수 있게 만드는
      // 표준 변환이다(perplexity). 정규화 엔트로피 h/log(n) 을 쓰다가 바꿨다 —
      // 그건 '자기가 쓴 캐릭 수'로 나누는 바람에 **두 캐릭 반반(1.00)과
      // 열 캐릭 고르게(1.00)가 같은 값**이 됐다. 이름이 '다양성'인데 폭이 아니라
      // 고르기만 재고 있었고, 0.58 같은 무단위 값이라 읽을 수도 없었다.
      // exp 를 취하면 둘이 2.0 대 10.0 으로 갈리고 단위(캐릭)가 생긴다.
      return Math.exp(h);
    },
    min: 1,
    max: 5,
    neutral: null,
    // 아래끝 1 은 정의에서 나온다 — 한 캐릭만 쓰면 수학적으로 정확히 1.0.
    // 위끝 5 는 관례다: 실질 5캐릭을 고르게 쓰면 사실상 주력이 없는 것이라
    // 그 위를 더 가를 이유가 없다. 중립은 없다 — 폭에 '본전'이랄 지점이 없다.
    scaleWhy: '고르게 썼다면 몇 캐릭인가(exp 엔트로피). 아래끝 1은 정의값, 위끝 5는 관례',
    fmt: (v) => `${v.toFixed(1)}캐릭`,
  },
  {
    key: 'grind',
    label: '몰입',
