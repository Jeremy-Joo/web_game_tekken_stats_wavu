...
import { fetchPlayerIndex, indexSummaryByPlayer, type PlayerIndexSummary } from '@/lib/tekken/player-index';
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // 메인 캐릭터·레이팅 — /api/admin/stats 와 같은 방식(플레이어 인덱스, 500판 이상만).
    // 못 받아와도 엑셀 자체는 만들어져야 하므로 catch 해서 빈 맵으로 떨어뜨린다.
    const idxSummary = await fetchPlayerIndex()
      .then(indexSummaryByPlayer)
      .catch(() => new Map<string, PlayerIndexSummary>());
    ...
    const tabs: TabData[] = [
      {
        ...
        label: '조회된 플레이어',
        columns: [
          '#', '마지막 조회', '이름', '메인', '레이팅', '식별코드', '조회', '페이지뷰', '깊이', '비율(%)',
          '사용자', '첫 조회', '조회일 수', '패턴', '언어',
        ],
        rows: players.map((p, i) => [
          i + 1, p.lastDate, p.name || '',
          idxSummary.get(p.id)?.mainChar ?? '', idxSummary.get(p.id)?.rating ?? '',
          p.id, p.lookups,
          p.views,
