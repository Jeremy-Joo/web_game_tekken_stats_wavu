...
const MOVIE_QUOTE_4: Record<'blazing' | 'hot' | 'steady' | 'cooling' | 'cold', Record<Lang, string[]>> = {
  blazing: {
    ko: [
      ...
      '정신 나간 것처럼 좋은 날입니다. (영화 매드맥스: 분노의 도로의 대사에서)',
      '"나는 아이언맨이다"급의 선언을 해도 되는 하루입니다. (영화 아이언맨의 대사에서)',
      '"예이 아드리안, 해냈어!"급으로 소리쳐도 되는 승률입니다. (영화 록키의 대사에서)',
      '"해적왕이 될 거다"급의 선언을 해도 되는 날입니다. (영화 원피스의 대사에서)',
    ],
    en: [
      'Even the ship they called unsinkable went down. This kind of streak is no different. (from a line in Titanic)',
      'What a lovely day. (from a line in Mad Max: Fury Road)',
      'This is "I am Iron Man" territory. (from a line in Iron Man)',
      'This is a "Yo Adrian, I did it!" kind of win rate. (from a line in Rocky)',
      'This is "I am going to be King of the Pirates" territory. (from a line in One Piece)',
    ],
    ja: [
      '神にも沈められないと言われた船も沈みました。今のこの勢いも同じです。(映画『タイタニック』のセリフから)',
      '最高の日じゃないか。(映画『マッドマックス 怒りのデス・ロード』のセリフから)',
      '「私はアイアンマンだ」レベルの宣言をしてもいい日です。(映画『アイアンマン』のセリフから)',
      '「エイドリアン、やったぞ!」と叫んでもいい勝率です。(映画『ロッキー』のセリフから)',
      '「海賊王に俺はなる!」レベルの宣言をしてもいい日です。(映画『ONE PIECE』のセリフから)',
    ],
  },
  hot: {
    ko: [
      '이것이 오늘 승률이다. (영화 300의 대사에서)',
      '지금까지 이런 폼은 없었다. (영화 극한직업의 대사에서)',
      '손모가지를 걸어야지 급의 배짱으로 딴 승률입니다. (영화 타짜의 대사에서)',
      '회피부동, 즉시 참수 수준의 기세입니다. (영화 명량의 대사에서)',
      '"속도에 대한 갈증"급의 승률입니다. (영화 탑건의 대사에서)',
    ],
    en: [
      "This is today's win rate! (from a line in 300)",
      "I'm kind of a big deal today. (from a line in Anchorman)",
      'This is "the need for speed" territory. (from a line in Top Gun)',
      'This is "Show me the money!" energy. (from a line in Jerry Maguire)',
      'This is "To infinity and beyond!" territory. (from a line in Toy Story)',
    ],
    ja: [
      'これが今日の勝率だ。(映画『300』のセリフから)',
      '戦闘力で例えるなら、今日は53万です。(映画『ドラゴンボールZ』のセリフから)',
      '「スピードへの渇望」レベルの勝率です。(映画『トップガン』のセリフから)',
      '「俺より強い奴に会いに行く」レベルの自信です。(映画『ドラゴンボールZ』のセリフから)',
      '「無限の彼方へ、さあ行くぞ!」レベルの気分です。(映画『トイ・ストーリー』のセリフから)',
    ],
  },
  steady: {
    ko: [
      '인생은 초콜릿 상자 같다는데, 오늘 승률은 열기 전부터 뭐가 나올지 다 압니다. (영화 포레스트 검프의 대사에서)',
      '우리는 답을 찾을 것이다, 늘 그랬듯이 — 다만 오늘은 아직 그 답을 못 찾은 하루입니다. (영화 인터스텔라의 대사에서)',
      '계획이 다 있다기엔, 오늘은 그냥 계획대로만 흘러갔습니다. (영화 기생충의 대사에서)',
      '"숟가락은 없다"는 대사처럼, 오늘 승률도 딱히 특별할 게 없습니다. (영화 매트릭스의 대사에서)',
    ...
    en: [
      ...
      'We will find a way. We always have. Just not today. (from a line in Interstellar)',
      '"There is no spoon" — and there is not much of a story in today\'s win rate either. (from a line in The Matrix)',
      '"It is not personal, it is strictly business." Today\'s numbers felt about that detached. (from a line in The Godfather)',
      'Life moves pretty fast — today just did not have much to stop and look at. (from a line in Ferris Bueller\'s Day Off)',
    ],
    ja: [
      '人生はチョコレートの箱のようなものだそうですが、今日の勝率は開ける前から中身がわかります。(映画『フォレスト・ガンプ』のセリフから)',
      '答えはいつか見つかります。いつもそうだったように。ただ、今日はまだです。(映画『インターステラー』のセリフから)',
      '「スプーンは存在しない」と言いますが、今日の勝率にも大した話はありません。(映画『マトリックス』のセリフから)',
      '「個人的な恨みじゃない、ただのビジネスだ」というほど、今日の数字も淡々としていました。(映画『ゴッドファーザー』のセリフから)',
      '「人生は思ったより早く過ぎる」と言いますが、今日は特に立ち止まるようなこともありませんでした。(映画『フェリスはある朝突然に』のセリフから)',
    ],
  },
  cooling: {
    ko: [
      '밤이 가장 어두울 때 새벽이 가장 가깝다는데, 지금은 아직 밤입니다. (영화 다크나이트의 대사에서)',
      '묻고 더블로 가는 오늘은 하지 마세요. (영화 타짜의 대사에서)',
