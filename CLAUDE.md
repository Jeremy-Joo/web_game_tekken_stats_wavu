...
**코드가 통과해도 말이 틀리면 나가면 안 된다.** 새로 만들거나 고친 문구를 전부 모아
아래를 확인한다. 검사 스크립트가 못 잡는 부류라 사람이 봐야 한다.

- **품사·격을 주변과 맞춘다.** 리포트 절 제목은 전부 명사구다(`최근 흐름`·`레이팅 추이`·
  `캐릭터별 성적`). 사고 이력: 활동 히트맵 절에 `언제 쳤나` 를 넣었는데 혼자 의문문이었고
  '치다'는 구어였다 → `날짜별 활동`.
- **같은 것을 같은 말로 부른다.** 경기/판, 캐릭터/캐릭 처럼 섞이기 쉽다.
  한 화면 안에서는 하나로 통일한다.
- **숫자에 단위를 붙인다.** `4최장 연속` 처럼 단위가 빠지면 무엇의 4인지 알 수 없다.
  `추세: 1판 늘 때 -0.78` 도 무엇의 -0.78 인지 안 밝혀서 `레이팅 -0.78` 로 고쳤다.
- **단정할 수 없는 것을 단정하지 않는다.** 표본 밖인데 `상위 0%`, 근거 없는데 `하위권`.
  못 내는 값은 **아예 안 내는 것**이 맞다.
- **사람에게 딱지를 붙이지 않는다.** `app/jokes.ts` 의 톤 규칙과 같은 선이다 —
  대상은 성적이지 사람이 아니다.
- **세 언어를 같이 고친다.** 한국어만 고치면 en/ja 가 조용히 옛말로 남는다.
- 아이콘·이모지는 `app/i18n.ts` 머리 주석의 규칙을 따른다
  (버튼이 여럿 붙은 줄에는 두지 않는다).
...
- `rank`(단)·`stage_id` 는 숫자만 오고 wavu 가 이름을 어디에도 안 쓴다(전문 검색 확인).
  그래서 단은 매핑을 안 만들었고, 스테이지는 수집만 하고 집계하지 않는다 —
  스테이지별 승률 탭을 한 번 만들었다가 `#500 70.14%` 처럼 읽을 수 없어 뺐다.
  **매핑이 확보되기 전에는 만들지 말 것**이라는 규칙이 여기서 나왔다.

  > **2026-08-04 — 그 조건이 충족됐다.** 둘 다 매핑을 확보하고 검증까지 마쳤다.
  > 전말과 근거는 [docs/population-features.md](docs/population-features.md) 5장.
  >
  > - **단: 38단계(0~37)** 로 확정. 출처는 ewgf 저장소지만 **그대로 믿지 않았다** —
  >   179,176건 관측으로 "시즌1 최대가 정확히 29(30종)"를 확인해 자체 검증했다.
  >   우리 표본에 37(파괴신 ∞)은 안 잡힌다. 너무 드물어서다(36이 이미 0.03%).
  > - **스테이지: 16종 확정 + 6종.** 확정 16종은 ewgf 백엔드 `tekken_enums.json` 의
  >   키 집합과 우리 실측 출시분이 **정확히 일치**한다. 나머지 6종은 최초 등장일을
  >   DLC 출시일과 맞춰 채웠고, 그중 **1700·1800/1801 은 추론이다**(규칙 기반).
  >   **200/201(Urban Square 낮/저녁)은 출처끼리 뒤집혀 있다** — 그 구분을 화면에
  >   낼 거라면 먼저 풀어야 한다.
  >
  > **규칙은 그대로 유지한다.** 확보되지 않은 것(1700·1800·200/201 의 낮밤)은
  > 여전히 추측으로 잇지 말 것. 그리고 **매핑이 생겼다는 게 만들어야 한다는 뜻은 아니다** —
  > 넣을지는 별도 판단이고 아직 안 정했다.
